...
function ReceptionScene({original}:{original:Content}){
 ...
 return <>
  ...
  <main className="scene" ref={scene} data-intro={revealing?'reveal':undefined} data-menu-returning={menuReturning||undefined} onPointerDownCapture={finishReveal} onFocusCapture={finishReveal}>
   <header className="topline"><a className="brand-mark" href={'/?lang='+locale} aria-label={t("Zuby Dásně — úvodní stránka")}><img src="/zuby-dasne-logo.png" alt="Zuby Dásně"/></a><div className="topline-actions"><LanguageSwitcher/><a href={'tel:'+data.contact.phone.replace(/\s/g,'')}><Phone size={15}/>{data.contact.phone}</a></div></header>
   <div className="stage">
