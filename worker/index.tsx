...
function html(element:React.ReactNode,bootstrap:unknown,title='ZUBY | DÁSNĚ — stomatologické centrum v Hradci Králové'){
 ...
 return new Response(`<!doctype html><html lang="cs"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><meta name="description" content="ZUBY | DÁSNĚ — informace o ordinaci, náš tým, průběh léčby, ceník a kontakt."><link rel="icon" href="/favicon.svg"><link rel="stylesheet" href="/style.css"><link rel="preload" href="/source/ISOCPEUR%20Regular.ttf" as="font" type="font/ttf" crossorigin></head><body><div id="root">${body}</div><script id="bootstrap" type="application/json">${payload}</script><script type="module" src="/client.js"></script></body></html>`,{headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store','X-Content-Type-Options':'nosniff','Referrer-Policy':'strict-origin-when-cross-origin','Content-Security-Policy':"default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' blob: data:; font-src 'self'; connect-src 'self'; frame-ancestors 'self' https://chatgpt.com https://*.chatgpt.com; base-uri 'self'; form-action 'self'"}});
}
export default {async fetch(req:Request,bindings:Bindings){return requestContext.run(req,async()=>{try{
 ...
 if(!status.user)return html(<main className="login"><h1>Správa obsahu</h1><p>Přihlaste se pro úpravu webu Zuby Dásně.</p><a className="primary" href="/signin-with-chatgpt?return_to=/admin" target="_top">Přihlásit se</a><p><a href="/">Zpět na web</a></p></main>,{page:'none'},'Správa obsahu | Zuby Dásně');
