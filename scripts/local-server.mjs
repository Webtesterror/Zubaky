...
const server=http.createServer(async(incoming,out)=>{try{
 ...
 const supplied=new URLSearchParams(body.toString()).get('code')||'';if(supplied.length!==code.length||!crypto.timingSafeEqual(Buffer.from(supplied),Buffer.from(code))){out.writeHead(403,{'Content-Type':'text/html;charset=utf-8'});out.end('<p>Nesprávný kód.</p><a href="/signin-with-chatgpt">Zkusit znovu</a>');return}
