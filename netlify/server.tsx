...
type State={draft:typeof seed,published:typeof seed,revision:number};
...
function document(body:string,bootstrap:unknown,status=200){
 return new Response(`<!doctype html><html lang="cs"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ZUBY | DÁSNĚ — Hradec Králové</title><meta name="description" content="ZUBY | DÁSNĚ — informace o ordinaci, náš tým, průběh léčby, ceník a kontakt."><link rel="icon" href="/favicon.png"><link rel="stylesheet" href="/style.css"></head><body><div id="root">${body}</div><script id="bootstrap" type="application/json">${JSON.stringify(bootstrap).replace(/</g,'\\u003c')}</script><script type="module" src="/client.js"></script></body></html>`,{status,headers:{...headers,'Content-Type':'text/html;charset=utf-8','Content-Security-Policy':"default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' blob: data:; font-src 'self'; connect-src 'self'; frame-src https://www.google.com https://maps.google.com; frame-ancestors 'self'; base-uri 'self'; form-action 'self'"}});
}
